...
void ArbolB::insertar(int clave, NodoGrafo* nodo_grafo) {
    ...
    while (!nodo_actual->es_hoja()) {
        ...
        while (indice_puntero < cantidad_claves && claves[indice_puntero] <= clave) indice_puntero++;
    ...
    if (nodo_hoja->insertar_clave(clave)) {
        ...
        while (posicion_insercion < cantidad_claves && claves[posicion_insercion] != clave) posicion_insercion++;
    ...
    int posicion_insercion = 0;
    while (posicion_insercion < cantidad_claves && claves_temporales[posicion_insercion] < clave) posicion_insercion++;
    ...
    for (int i = 0; i < nodo_hoja->obtener_orden(); ++i) nodo_hoja->establecer_dato(i, 0);
    while (nodo_hoja->obtener_num_claves() > 0) nodo_hoja->decrementar_num_claves();
...
NodoGrafo* ArbolB::buscar(int clave) {
    ...
    int* claves = nodo_hoja->obtener_claves();
    int cantidad_claves = nodo_hoja->obtener_num_claves();
    for (int i = 0; i < cantidad_claves; ++i) if (claves[i] == clave) return nodo_hoja->obtener_dato(i);
    return 0;
}
...
bool ArbolB::eliminar(int clave) {
    ...
    for (int i = 0; i < cantidad_claves; ++i) if (claves[i] == clave) { posicion_eliminar = i; break; }
    if (posicion_eliminar == -1) return false;
